...
func day2() {
	...
	for _, line := range strings.Split(day2Input, "\n") {
		if line == "" {
			continue
		}

		parts := strings.Split(line, ": ")
		gameIdString := strings.Split(parts[0], "Game ")[1]
		gameId, err := strconv.Atoi(gameIdString)

		if err != nil {
			panic("Could not parse game id: " + gameIdString)
		}
		...
		for _, game := range games {
			...
			for _, part := range parts {
				...
				if err != nil {
					panic("Could not parse number: " + numStr)
				}
		...
		fmt.Printf("\tHighest Red: %d\n", highestRed)
		fmt.Printf("\tHighest Green: %d\n", highestGreen)
		fmt.Printf("\tHighest Blue: %d\n", highestBlue)

		powerTotal += (highestRed * highestBlue * highestGreen)
		if allRoundsPass {
			gameIdSum += gameId
		}
	...
	fmt.Println("=======================================")
	fmt.Printf("Game Id Total: %d\n", gameIdSum)
	fmt.Printf("Power Total: %d\n", powerTotal)
