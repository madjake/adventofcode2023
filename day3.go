...
func day3Part1() {
	...
	for x, line := range lines {
		...
		for y := 0; y < len(line); y++ {
			...
			partNumber, numEnd, valid := getNumber(x, y, lines)
			y = numEnd

			if valid {
				validNumbers = append(validNumbers, partNumber)
				...
				invalidNumbers = append(invalidNumbers, partNumber)
			}
		}
	}

	fmt.Printf("====================\nValid numbers: %+v\nInvalid numbers: %+v\nTotal Part Numbers: %d\n", validNumbers, invalidNumbers, totalPartNumbers)
}
...
func day3Part2() {
	...
	lines := strings.Split(day3Input, "\n")
	numbers = append(numbers, matchNumbers(lines)...)
	gears = append(gears, matchRunes('*', lines)...)

	gearMapping := map[string][]int{}

	for _, gear := range gears {
		gearMapping[fmt.Sprintf("%d,%d", gear.x, gear.y)] = []int{}
	}

	for _, number := range numbers {
		l := len(strconv.Itoa(number.num))
		found := false
		for i := 0; i < l; i++ {
			if found {
				found = false
			...
			positions := [][]int{
				{x - 1, number.y},     // west
				{x - 1, number.y - 1}, // northwest
				{x, number.y - 1},     // north
				{x + 1, number.y + 1}, // southwest
				{x + 1, number.y},     // east
				{x + 1, number.y - 1}, // southeast
				{x, number.y + 1},     // south
				{x - 1, number.y + 1}, //northeast
			}

			for _, pos := range positions {
				k := fmt.Sprintf("%d,%d", pos[0], pos[1])

				if _, ok := gearMapping[k]; ok {
					gearMapping[k] = append(gearMapping[k], number.num)
					found = true
				}
			}
		}
	...
	for _, gear := range gearMapping {
		if len(gear) > 2 {
			panic("more than 2 gears")
		}
...
func isPartNumberValid(x int, y int, length int, lines []string) bool {
...
func getNumber(x int, y int, lines []string) (int, int, bool) {
	fullNum := 0
	pos := 0
	line := lines[x]
	i := y
	for ; i < len(line); i++ {
		...
		if r >= '0' && r <= '9' {
			n := int(r - '0')
			fullNum = fullNum*10 + n
			pos++
		} else {
			return fullNum, i, isPartNumberValid(x, y, pos, lines)
		}
	}
